...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run LLaVA baseline QA testing")
    parser.add_argument(
        "--level",
        type=int,
        required=True,
        help="Level number (0-6)",
    )
    parser.add_argument(
        "--id",
        type=str,
        required=True,
        help="Image ID (e.g., 00000_b)",
    )
    parser.add_argument(
        "--strategy",
        type=int,
...
def main():
    ...
    print("=== BASELINE QA TESTING ===")
    if args.all_strategies:
        print(f"Testing LLaVA on existence questions using ALL strategies (0-3) to verify basic object recognition.")
    else:
        print(f"Testing LLaVA on existence questions (strategy {args.strategy}) to verify basic object recognition.")
    print("=" * 50)

    # Set defaults for baseline testing
    args.num_questions = None  # Use all baseline questions
    args.plot_attention = False
    args.plot_trends = False
    args.plot_relational_phrase_attention = False

    # Allow args to override globals
    global MODEL_ID
    if args.model_id:
    ...
    # Load Model once (shared across strategies)
    ...
    # Data Prep (shared across strategies)
    ...
    # Extract per-object patch indices directly from JSON (shared across strategies)
    ...
    REQUIRES_ATTENTION = False  # Disabled for baseline QA testing
    print("NOTE: Attention extraction is DISABLED for baseline QA testing.")
    ...
    for strategy in strategies_to_run:
        ...
        image_output = os.path.join(level_output, f"{args.id}")
        strategy_output = os.path.join(image_output, f"strategy_{strategy}_{timestamp}")

        # Create subdirectories
        overview_dir = os.path.join(strategy_output, "overview")
        trends_dir = os.path.join(strategy_output, "trend_analysis")
        attention_dir = os.path.join(strategy_output, "full_attention")
        phrase_attn_dir = os.path.join(strategy_output, "phrase_attention")
        ...
        # Load strategy-specific QA
        qa_key = f"qa_baseline_{strategy}"
        full_qa_list = annotation_data.get(qa_key, [])
        if not full_qa_list:
            print(f"Warning: No QA data found for strategy {strategy} (key: {qa_key}). Skipping.")
            continue
        ...
        if results_summary:
            plot_evaluation_results(results_summary, overview_dir, f"{args.id} (Level {args.level}, Strategy {strategy})")
        
        # --- GENERATE THE COMPARISON PLOT ---
        if args.plot_trends:
            plot_correct_vs_incorrect_trends(correct_runs_layerwise, incorrect_runs_layerwise, overview_dir)
            
        print(f"\nStrategy {strategy} completed. Results saved to: {os.path.abspath(strategy_output)}")

    print(f"\n{'='*50}")
    print("ALL STRATEGIES COMPLETED" if args.all_strategies else f"STRATEGY {args.strategy} COMPLETED")
    print(f"{'='*50}")
