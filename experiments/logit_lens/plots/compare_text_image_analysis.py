...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out_dir",
        type=str,
        default=os.path.join(_NEW_RESULTS_ROOT, "analysis_compare_text_image"),
    )
...
def main() -> None:
    ...
    for lvl in levels:
        ...
        img_conf = _confusion_counts(img_records, "answer", "prediction")
        ...
        for r in img_records:
            ...
            if isinstance(conf, (int, float)) and isinstance(conf_no, (int, float)):
                c = max(float(conf), float(conf_no))
                if r.get("is_correct") is True:
                    img_conf_correct.append(c)
                elif r.get("is_correct") is False:
                    img_conf_wrong.append(c)
